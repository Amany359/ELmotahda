...
<head>
    ...
    <title>{{__("lan.Forgot Password")}}</title>
    <link href="{{url('auth/css/bootstrap.min.css')}}" rel="stylesheet">
    <link href="{{url('auth/css/common.css')}}" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Nunito:400,600,700&amp;display=swap" rel="stylesheet">
    <link href="{{url('auth/css/theme-06.css')}}" rel="stylesheet">
</head>
...
<body>
                    ...
                    <div class="forny-form">
                        <div class="reset-form d-block">
                            <form class="reset-password-form">
                                <h4 class="mb-5">{{__("lan.reset")}}</h4>
                                <p class="mb-10">
                                    {{__("lan.required_Email")}}
                                </p>
                                <div class="form-group">
                                    <div class="input-group">
                                        ...
                                        <input required class="form-control login_inputs" name="email" type="email" placeholder="{{__("lan.Type Your Email")}}">
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-12">
                                        <button class="btn btn-primary btn-block" onclick="send_mail()">{{__("lan.send")}}</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                ...
                <div class="right-pane">
                    <div class="text-center" style="width: 300px; margin-top:-140px">
                        <div class="mb-10 forny-logo">
                            <img src="{{url('auth/img/logo-06.svg')}}">
                        </div>
                        <div class="mt-8">
                            <h4 class="mb-4">{{__("lan.welcome")}}</h4>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script src="{{url('auth/js/demo.js')}}"></script>
    ...
    <script>
        function send_mail (){
            let login_inputs = document.querySelector(".login_inputs");
            
            let data_form = new FormData($("#signin")[0]);
            $.ajax({
                type:"post",
                enctype : "multipart/form-data",
                url: "{{   route('sendlinkaccess')   }}",
                data: {
                    "_token" : "{{csrf_token()}}",
                    "email" : login_inputs.value,
                },
